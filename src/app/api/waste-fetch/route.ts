...
const WASTE_TYPE_MAP: Record<string, string> = {
  ...
  biotonne: 'Biotonne', bioabfall: 'Biotonne', biomüll: 'Biotonne',
...
function normalizeCityForAWIDO(city: string): string[] {
  ...
  return [...new Set([base, base.split('-')[0], base.replace(/-/g, ''), `${base}-land`, `kreis-${base}`])].filter(Boolean);
}

async function tryAWIDO(city: string): Promise<{ entries: any[]; client: string } | null> {
  ...
  for (const candidate of candidates) {
    try {
      const ortsRes = await fetch(`https://${candidate}.awido-online.de/WebServices/GetOrtschaften.ashx`, {
        signal: AbortSignal.timeout(4000),
      });
      if (!ortsRes.ok) continue;
      const orts: any[] = await ortsRes.json();
      if (!Array.isArray(orts) || !orts.length) continue;

      const cityLower = city.toLowerCase();
      const match = orts.find((o) => o.scene?.toLowerCase() === cityLower)
        || orts.find((o) => o.scene?.toLowerCase().includes(cityLower.substring(0, 4)))
        || orts[0];

      const datesRes = await fetch(
        `https://${candidate}.awido-online.de/WebServices/GetPickupDates.ashx?sTyp=&sOrt=${match.key}&sStr=&sHnr=&dStart=${today}&dEnd=${end}`,
      ...
      const entries = raw
        .map((d) => ({ date: d.dt, type: normalizeWasteType(d.anl_34_art || d.art || ''), note: '' }))
...
async function tryICalFeed(icalUrl: string): Promise<any[] | null> {
  try {
    ...
    for (const block of text.split('BEGIN:VEVENT').slice(1)) {
      ...
      const type = normalizeWasteType(summaryMatch[1].trim().replace(/\r/g, ''));
      if (!type || date < today) continue;
...
async function fallbackAI(city: string, zipCode: string, state: string): Promise<any[]> {
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `Erstelle einen plausiblen deutschen Müllkalender für 8 Wochen ab heute.
Restmüll alle 2 Wochen, Biotonne wöchentlich (Sommer)/2-wöchentlich (Winter), Gelbe Tonne alle 2 Wochen, Altpapier alle 4 Wochen. Nur werktägliche Termine (Mo-Fr).
JSON: {"entries":[{"date":"YYYY-MM-DD","type":"Restmüll","note":"KI-Schätzung"}]}
Gültige types: Restmüll, Biotonne, Gelbe Tonne, Altpapier, Sperrmüll, Glas`,
      },
      { role: 'user', content: `${city}, PLZ ${zipCode}, ${state}. Heute: ${today}` },
    ],
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1. Manually provided iCal URL
    if (icalUrl?.trim()) {
      const entries = await tryICalFeed(icalUrl.trim());
      if (entries) return NextResponse.json({ entries, source: 'ical', sourceLabel: 'iCal-Kalender' });
    }

    // 2. Abfallplus (K4Systems) — AWB Rastatt + ~200 municipalities
    if (city && state) {
      const result = await fetchAbfallplus(city, state);
      if (result) {
        return NextResponse.json({
          entries: result.entries,
          source: 'abfallplus',
          sourceLabel: `Abfallplus · ${result.municipality}`,
        });
      }
    }

    // 3. AWIDO — ~400 other municipalities
    const awidoResult = await tryAWIDO(city || '');
    if (awidoResult) {
      return NextResponse.json({
